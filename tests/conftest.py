...
@pytest.fixture(scope="session") # The fixture runs only once per test session (entire pytest run)
def account():
...
@pytest.fixture(scope="function") # The fixture runs once per test function that uses it
def coffee(eth_usd):   
